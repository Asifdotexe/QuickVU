...
st.markdown("""Quick Prep is a versatile data cleaning tool to help 
prepare your dataset for analysis. Simply upload your data, 
select the desired cleaning options, and download the prepared data.""")
...
if uploaded_file:
    ...
    st.sidebar.markdown('<h3 class="side-header">Data Cleaning Options</h3>',
                        unsafe_allow_html=True)
    ...
    if st.sidebar.checkbox("Detect and Handle Outliers",
                           help="Select columns and choose a method to detect "
                                "and handle outliers in the dataset."):
        outlier_columns = st.sidebar.multiselect("Select Columns for Outlier Treatment",
                                                 df.select_dtypes(include=[np.number]).columns)
        outlier_method = st.sidebar.radio("Outlier Detection Method",
                                          ("Z-score", "IQR"))
        apply_outliers = st.sidebar.button("Apply Outlier Treatment",
                                           help="Marks outliers as TRUE, non-outliers as FALSE.")
    ...
    if st.sidebar.checkbox("Scale Data",
                           help="Select a column to scale and choose the "
                                "scaling method (Standardize or Normalize)."):
        scale_column = st.sidebar.selectbox("Select Column to Scale",
                                            df.select_dtypes(include=[np.number]).columns)
        scaling_method = st.sidebar.radio("Scaling Method",
    ...
    if st.sidebar.checkbox("Drop Columns",
                           help="Select and remove a column from the dataset."):
    ...
    if st.sidebar.checkbox("Change Data Type",
                           help="Change the data type of a selected column."):
        dtype_column = st.sidebar.selectbox("Select Column to Change Type",
                                            df.columns)
    ...
    if st.sidebar.checkbox("Filter Rows",
                           help="Filter rows based on selected values "
                                "from a specific column."):
        filter_column = st.sidebar.selectbox("Select Column to Filter By",
                                             df.columns)
        unique_values = df[filter_column].unique()
        filter_values = st.sidebar.multiselect("Select Values to Keep",
                                               unique_values)
        if filter_values:
    ...
    # Download option
    st.sidebar.markdown('<h3 class="side-header">Download Cleaned Data</h3>',
                        unsafe_allow_html=True)
    ...
    st.markdown('<p class="instructions">Please upload a file to start '
                'data cleaning. </br> You can drop the file at the data '
                'uploader located on the navigation side bar. </p>',
                unsafe_allow_html=True)
