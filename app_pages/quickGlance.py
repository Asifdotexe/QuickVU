...
if uploaded_file:
    ...
    st.sidebar.markdown('<h3 class="side-header">Select Columns for Analysis</h3>',
                        unsafe_allow_html=True)
    selected_categorical = st.sidebar.multiselect("Select Categorical Columns",
                                                  categorical_columns,
                                                  help="Choose columns with categorical data "
                                                       "like 'Product Type', 'Category', etc.")
    selected_numerical = st.sidebar.multiselect("Select Numerical Columns",
                                                numerical_columns,
                                                help="Select columns with "
                                                     "numerical data like 'Sales Amount', 'Profit', etc.")
    ...
    if st.sidebar.checkbox("Show Summary Statistics",
                           help="Display summary statistics such as mean, "
                                "median, and standard deviation for numerical columns."):
    ...
    if st.sidebar.checkbox("Show Correlation Matrix",
                           help="Display a correlation matrix for "
        ...
        if selected_numerical:
            ...
            sns.heatmap(correlation_matrix, annot=True, fmt=".2f",
                        ...
                        ax=ax)
            ax.set_title('Correlation Matrix of Selected Numerical Columns',
    ...
    st.sidebar.markdown('<h3 class="side-header">Data Visualization</h3>',
                        unsafe_allow_html=True)
    
    if st.sidebar.checkbox("Plot Metrics by Category",
        ...
        if selected_categorical and selected_numerical:
            ...
            numer_col = st.sidebar.selectbox("Select Numeric Column",
                                             selected_numerical,
            ...
            st.markdown('<p class="warning-message">Please select both a '
                        'Categorical and a Numerical column for this analysis.</p>',
                        unsafe_allow_html=True)

    # Plot by dates
    if st.sidebar.checkbox("Plot Metrics Trends",
                           help="Plot metric trends over time based on "
                                "selected columns"):
        st.write("## Metrics Over Time")

        datetime_columns = df_clean.select_dtypes(include=['datetime64[ns]',
                                                           'object']).columns.tolist()

        if datetime_columns and numerical_columns:
            date_col = datetime_columns[0] if len(datetime_columns) == 1 else (
                st.sidebar.selectbox("Select Date Column", datetime_columns))
            amount_col = st.sidebar.selectbox("Select Numeric Column",
                                              selected_numerical)
            ...
            st.warning("Please ensure your dataset contains both Date/Time "
                       "and Numerical columns for this analysis.")
    ...
    st.markdown('<p class="instructions">Please upload a CSV file to start '
                ...
                'located on the navigation side bar. </p>',
                unsafe_allow_html=True)
